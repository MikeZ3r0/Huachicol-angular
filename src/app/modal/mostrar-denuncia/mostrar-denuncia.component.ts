  ...
  sesion= false;
    ...
    getDenuncia(){
      this.mostrarDenunciaService.getDenuncia().subscribe( datos => {
        ...
        console.log("denuncias "+denunciaJSON[0]._id);
        for(let i=0; i< denunciaJSON.length; i++){
          this.denuncia = new Denuncia(denunciaJSON[i].titulo, denunciaJSON[i].descripcion, denunciaJSON[i].sendDate, denunciaJSON[i].punto.coordinates,  denunciaJSON[i].userId,denunciaJSON[i]._id);
